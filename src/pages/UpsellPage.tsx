...
export function UpsellPage({ onSkipToStarter, onSuccessVIP, onSkip }: UpsellPageProps) {
  ...
  const ALL_MODELS: ModelItem[] = useMemo(() => {
    ...
    return [...scannedModels, ...DEDICATED_FURNITURE, ...normalizedRaw(rawModels)];
  }, []);
...
// Helper to normalize raw models
function normalizedRaw(rawModels: any[]): ModelItem[] {
  return rawModels.map((model, idx) => ({
    id: model.id || `public_${idx}`,
    title: model.title || "Untitled",
    rawName: model.rawName || model.fileName || `MODEL_${idx}.SKP`,
    categoryKey: model.categoryKey || "apartment",
    categoryName: model.categoryName || "Apartment",
    subCategoryKey: model.subCategoryKey,
    src: model.relPath || model.src || "/placeholder.jpg",
    renderEngine: model.renderEngine || "V-Ray 6",
    format: "SketchUp (.SKP)",
    polyCount: model.polyCount || "~120K Polys",
    fileSize: model.fileSize || "~55 MB",
