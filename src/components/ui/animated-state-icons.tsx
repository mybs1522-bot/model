...
/* ─── 6. BELL → NOTIFICATION ─── bell rings then green dot appears */
export function NotificationIcon({ size = 40, color = "currentColor", className, duration = 2800 }: StateIconProps) {
  ...
  return (
    <motion.svg viewBox="0 0 40 40" fill="none" className={cn("", className)}
      ...
      <motion.circle cx="28" cy="10" r="4" fill="#10b981"
        animate={notif
...
/* ─── 7. HEART → FILLED ─── heart fills green with bounce */
    ...
    <div className="w-full max-w-3xl mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h2 className="text-xl font-bold tracking-tight text-white mb-1">
          Animated State Micro-Icons
        </h2>
        <p className="text-xs text-slate-400 max-w-md mx-auto">
          12 micro-animating SVG icons in Emerald Green & White — morphing between active states.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4 justify-items-center">
        {ALL_ICONS.map(({ name, Icon }) => (
          <div key={name} className="flex flex-col items-center gap-2">
            <div className="flex items-center justify-center size-16 rounded-2xl border border-[#1e2330] bg-[#12141c] shadow-sm text-[#10b981]">
              <Icon size={28} color="#10b981" />
            </div>
            <span className="text-[10px] font-bold text-slate-400 tracking-wide text-center leading-tight">
              {name}
            </span>
          </div>
        ))}
