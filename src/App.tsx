...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-900 flex flex-col font-sans selection:bg-[#10b981] selection:text-black">
      ...
      >
        <div className="bg-slate-900 rounded-2xl p-2.5 sm:p-3 shadow-2xl flex items-center justify-between gap-3 w-full max-w-md sm:max-w-lg border border-slate-700 pointer-events-auto">
          <div className="flex flex-col text-left pl-1.5 sm:pl-2 min-w-0">
            <span className="text-white font-bold text-xs sm:text-sm whitespace-nowrap">
              7-Day Free Trial ($20/mo after)
            </span>
            <span className="text-emerald-400 font-mono text-[10px] sm:text-xs font-bold flex items-center gap-1 whitespace-nowrap">
              <Timer className="size-3 shrink-0" /> Offer Ends in {timerString}
            </span>
          </div>
          <button 
            onClick={() => setShowCheckoutModal(true)}
